...
def CATCH_timer(est_config='overlaps', loc_config='small_noisy', numimgs=200, num_trials=10, loc_dataset='../datasets/yolo_noisy/eval/', est_dataset='../datasets/overlaps/eval/'):
...
est = Estimator(configuration='{}')
loc = Localizer(configuration='{}')
catch = CATCH(localizer=loc, estimator = est)
'''.format(est_config, loc_config)
